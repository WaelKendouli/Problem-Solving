...
stDate GetDateFromDayOrder(short year, short PastDaysInYear,short AddedDaysByUser)
{
	stDate Date;
	Date.year = year;
	Date.mounth = 1;
	short RemainigDays = PastDaysInYear+AddedDaysByUser;
	short MounthDays = 0;
...
void ShowDate(int year, short Mounth, short Days,short AddedDaysByUser)
{
	stDate Date;
	 
	short DaysInPastMounths = PastDaysInYear(year, Mounth, Days);
	Date = GetDateFromDayOrder(year, DaysInPastMounths,AddedDaysByUser);
	cout << "Date after adding [" <<  AddedDaysByUser << "] days : " << Date.day << "/" << Date.mounth << "/" << Date.year;
}
...
int main()
{
	...
	short AddedDaysByUser = ReadNumber(" Number of Days You Want To add");

	cout << "\n\n";
	ShowDate(year, mounth, day,AddedDaysByUser);
	system("pause>0");
}
