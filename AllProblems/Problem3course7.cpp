...
void PrintRowSumsInArray(int tab[3])
{

	for (int i = 0; i < 3; i++)
	{
		cout << "Sum of Row " << i + 1 << " = " << tab[i] << endl;
	}

}
void FillAndPrintRowSumsInArray(short int RandomMatrix[3][3])
{
	int tab[3];
	for (int i = 0; i < 3; i++)
	{
		tab[i]= SumRowInRandomMatrix(RandomMatrix, i, 3) ;
	}
	PrintRowSumsInArray(tab);
}
...
int main()
{
	...
	FillAndPrintRowSumsInArray(RandomMatrix);
